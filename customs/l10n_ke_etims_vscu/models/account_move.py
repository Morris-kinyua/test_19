...
class AccountMove(models.Model):
    ...
    def _l10n_ke_oscu_json_from_move(self):
        ...
        content = {
            ...
            **tax_rates,
            ...
            'itemList':         line_items,
            'remark':           self._get_disbursement_remark(),           # Disbursement remark
    ...
    def _l10n_ke_get_invoice_sequence(self):
        ...
        sequence_code = 'l10n.ke.etims.sale.sequence' if self.is_sale_document(include_receipts=True) else 'l10n.ke.etims.purchase.sequence'
    ...
    def _l10n_ke_oscu_send_customer_invoice(self):
        """Send customer invoice to KRA in real-time using eTIMS API."""
        company = self.company_id
        
        if not company.l10n_ke_oscu_cmc_key:
            raise UserError(_("Device is not initialized. Please initialize the device first."))
        ...
        # Use new API client for transmission
        from .etims_api_client import EtimsApiClient
        client = EtimsApiClient(company)
        
        success, data, error = client.call('saveTrnsSalesOsdc', content)
        
        if success:
            self.write({
                'l10n_ke_oscu_receipt_number': data.get('curRcptNo'),
                'l10n_ke_oscu_invoice_number': content['invcNo'],
                'l10n_ke_oscu_signature': data.get('rcptSign'),
                'l10n_ke_oscu_datetime': parse_etims_datetime(data.get('sdcDateTime')),
                'l10n_ke_oscu_internal_data': data.get('intrlData'),
                'l10n_ke_control_unit': data.get('sdcId', company.l10n_ke_control_unit),
            })
            self.message_post(body=_("Invoice successfully sent to KRA eTIMS."))
        else:
            # Decrement sequence on failure
            self._l10n_ke_get_invoice_sequence().number_next -= 1
            self.l10n_ke_oscu_invoice_number = False
            raise UserError(_("Failed to send invoice to KRA: %(message)s", message=error.get('message', 'Unknown error')))

    # === Sending to eTIMS: vendor bills === #

    def action_l10n_ke_oscu_confirm_vendor_bill(self):
        """Send vendor bill information to the KRA in real-time.
        ...
        for move in self:
            ...
            if move.l10n_ke_oscu_attachment_id:
                ...
                if not content.get('pmtTyCd'):
                    content['pmtTyCd'] = move.l10n_ke_payment_method_id.code or ''
            else:
            ...
            # Use new API client for real-time transmission
            client = EtimsApiClient(company)
            success, data, error = client.call('insertTrnsPurchase', content)

            if success:
                move.l10n_ke_oscu_invoice_number = content['invcNo']
                move.message_post(body=_("Purchase confirmed on KRA eTIMS."))
            else:
                # Decrement sequence on failure
                move._l10n_ke_get_invoice_sequence().number_next -= 1
                raise UserError(_("Failed to confirm purchase with KRA: %(message)s", 
    ...
    def _get_disbursement_remark(self):
        ...
        product_remarks = self.invoice_line_ids.filtered(
            lambda l: l.product_id and getattr(l.product_id, 'l10n_ke_remark', False)
        ).mapped('product_id.l10n_ke_remark')
        
        if product_remarks:
            return ', '.join(filter(None, product_remarks))[:400]
        return None
